...
- (void)windowDidResize:(NSNotification *)notification {
  DebugLog(@"Window resized to: %@",
           NSStringFromRect([((NSWindow *)notification.object) frame]));
}

- (void)windowDidMove:(NSNotification *)notification {
  DebugLog(@"Window moved to: %@",
           NSStringFromRect([((NSWindow *)notification.object) frame]));
}
...
public:
  ...
  void setupContentView() {
    ...
    NSTextView *textView =
        [[NSTextView alloc] initWithFrame:[scrollView bounds]];
  ...
  void positionWindow() {
    ...
    windowFrame.origin.x = screenFrame.origin.x + screenFrame.size.width -
  ...
  void *getNativeHandle() const { return (__bridge void *)window; }
...
OverlayWindow::OverlayWindow()
    : pImpl(std::make_unique<Impl>()), running(false) {
...
bool OverlayWindow::isRunning() const { return running; }
