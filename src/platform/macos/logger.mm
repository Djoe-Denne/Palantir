...
void platform_log(const char* function, int line, const char* format, ...) {
    ...
    char final_msg[1024];
    snprintf(final_msg, sizeof(final_msg), "%s:%d - %s", function, line, formatted_msg);
    
    // Convert to NSString and log
    NSString* nsMessage = [NSString stringWithUTF8String:final_msg];
    NSLog(@"%@", nsMessage);
} 