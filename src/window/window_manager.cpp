...
void WindowManager::addWindow(std::unique_ptr<IWindow> window) { windows.push_back(std::move(window)); }
...
auto WindowManager::getFirstWindow() const -> IWindow* { return windows.empty() ? nullptr : windows.front().get(); }
